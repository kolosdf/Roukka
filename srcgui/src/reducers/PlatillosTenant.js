...
export default function reducer(state = {
    ...
    switch (action.type) {
        ...
        case ActionTypes.SALDO_PLATILLO:
            const hola = (platillo) => {
                state.platillos.splice(state.platillos.indexOf(state.platillos.filter(platillon => platillon.id === platillo.id)[0]), 1, platillo);
            }
            console.log(action.payload.map(item => {
                hola(item.platillo)
            }))

            return {
                ...state, platillos: state.platillos
            }
